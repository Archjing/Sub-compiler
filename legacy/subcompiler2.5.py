...
class SpeechProcessor:
    ...
    def _to_ast(self, api_data: dict) -> SubtitleDocument:
        ...
        # 顺便把这次新发现的幻觉也拉黑
        blacklist = ["优优独播剧场", "YoYo Television", "Exclusive", "字幕由", "感谢观看", "明镜与点点", "点赞", "订阅"]
        
        for seg in api_data.get("segments", []):
            text = seg.get("text", "").strip()
            
            # 如果是空行，或者是黑名单里的幻觉，直接丢弃
            if not text or any(bad in text for bad in blacklist):
                continue
...
class ParserFactory:
    ...
    def _parse_json(c, doc):
        data = json.loads(c)
        for i in data:
            s, e = str(i.get("start", i.get("timestamp", [0,0])[0])), str(i.get("end", i.get("timestamp", [0,0])[1]))
